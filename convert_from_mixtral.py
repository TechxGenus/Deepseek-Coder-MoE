...
dense_config["model_type"] = "deepseek"
dense_config["aux_loss_alpha"] = dense_config["router_aux_loss_coef"]
dense_config["n_routed_experts"] = dense_config["num_local_experts"]
...
for name, param in sparse_model.named_parameters():
    ...
    elif map_param_name(name) in param_dict:
        param.data.copy_(param_dict[map_param_name(name)].data)
    else:
        continue
